	...
	public void addApplicationStateCreators(
		...
		final ApplicationStateCreator<G_User> currentUserCreator = new ApplicationStateCreator<G_User>() {
			...
			public G_User create() {
				...
				try {
				    // GBZ: With OAM integration, username may have to come from the OAMAuthCookie
					...
					if (ValidationUtils.isValid(user)) {
						if (user != null) {
							...
							// GBZ: with OAM integration, instead of calling isUserInRole(), we need to look for a cookie that contains the role information
