 ...
 */
package graphene.introspect.query.sql.codegen;
...
public class MetaDataIntrospector {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(MetaDataIntrospector.class);
	...
	public MetaDataIntrospector(DBConnectionPoolService cp) {
		this.cp = cp;
	}
	...
	protected Property createProperty(EntityType classModel, String columnName,
		...
		return new Property(classModel,
				namingStrategy.normalizeColumnName(columnName), propertyName,
				typeModel, Collections.<String> emptyList(), false);
	}
	 ...
	 * @throws SQLException
	 */
	public void introspect(DatabaseMetaData md) throws SQLException {
	...
	private void handleColumn(EntityType classModel, String tableName,
		...
		String propertyName = namingStrategy.getPropertyName(columnName,
				classModel);
		Class<?> clazz = configuration.getJavaType(columnType, columnSize,
				columnDigits, tableName, columnName);
		if (clazz == null) {
			throw new IllegalStateException("Found to mapping for "
					+ columnType);
		}
		...
		Property property = createProperty(classModel, columnName,
				propertyName, typeModel);
		if (columnAnnotations) {
			property.addAnnotation(new ColumnImpl(namingStrategy
	 ...
	 * 
	 * @param nameSuffix
	 *            name suffix for query-types (default: "")
	 */
	 ...
	 *            namingstrategy to override (default: new
