...
public class SampleDataTableModel implements DataTableModel {
	...
	public SampleDataTableModel(TypeCoercer typeCoercer,
			TranslatorSource translatorSource, ComponentResources cr) {
		super();
	...
	public GridDataSource filterData(GridDataSource source) {
		...
		return new GridDataSource() {
			...
			private CollectionGridDataSource cgds = new CollectionGridDataSource(
					datas);
	...
	public JSONObject getResponse(GridDataSource source) {
		...
		if (endIndex > records - 1)
			endIndex = records - 1;
		source.prepare(startIndex, endIndex + rowsPerPage,
		...
		for (int index = startIndex; index <= endIndex; index++) {
			...
			for (String name : names) {
				...
				if (name.equalsIgnoreCase("lastName")) {
					// val = "<a href=\""+cr.createPageLink(DataTables.class,
					// false, val)+"\">"+val+"</a>";
					val = "<p />";
				}
	...
	public JSONObject sendResponse(Request request, GridDataSource source,
			...
			PropertyOverrides overrides, boolean mode) {
