...
/**
 * Class used for creating a jQuery Widget You just have to create a class
 * extending this one. And automatically, the widget javascript method will be
 * called.
 * 
 * @since 2.6.0
 * 
...
public class Widget {
	...
	/**
	 * Name of the jQuery widget. Typically supplied by subclassing widget to a
	 * jQuery widget name.
	 */
	@Parameter(defaultPrefix = BindingConstants.LITERAL)
	...
	void afterRender() {
		String init = null;
		if (script != null) {
			init = String.format("%s('#%s').%s(%s);", jqueryAlias,
					clientElement.getClientId(), widgetName(), script);
			jsSupport.addScript(init);
		} else {
			init = String
					.format("%s('#%s').%s(%s);", jqueryAlias,
							clientElement.getClientId(), widgetName(),
							overrideParams());
