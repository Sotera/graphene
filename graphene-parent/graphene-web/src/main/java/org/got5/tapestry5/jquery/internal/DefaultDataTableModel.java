...
/**
 * A default DataTable model that handles ajax mode. Used for lazy loading and
...
public class DefaultDataTableModel implements DataTableModel {
	...
	public DefaultDataTableModel(TypeCoercer typeCoercer,
			TranslatorSource translatorSource, PageRenderQueue pageRenderQueue,
	...
	/**
	 * This method will filter all your data by using the search input from your
	 * datatable.
	 */
		...
		for (int index = 0; index < source.getAvailableRows(); index++) {
			...
			for (Object name : model.getPropertyNames()) {
				...
				try {
					...
					String val = (String) conduit
							.get(source.getRowValue(index));

					if (val.contains(request
							.getParameter(DataTableConstants.SEARCH)))
						flag = true;
				} catch (Exception e) {
	...
	public void prepareResponse(GridDataSource source) {
		...
		String sortingCols = request
				.getParameter(DataTableConstants.SORTING_COLS);
		...
		if (InternalUtils.isNonBlank(sortingCols)) {
			...
			if (nbSortingCols > 0) {
				...
				if (!(InternalUtils.isNonBlank(colSort.name()) && colSort
						.name().startsWith(sord.toUpperCase())))
	...
	public JSONObject getResponse(GridDataSource source) throws IOException {
		...
		String displayStart = request
				.getParameter(DataTableConstants.DISPLAY_START);
		int startIndex = FastNumberUtils.parseIntWithCheck(displayStart);

		String displayLength = request
				.getParameter(DataTableConstants.DISPLAY_LENGTH);
		...
		for (int index = startIndex; index <= endIndex; index++) {
			...
			for (String name : names) {
				...
				if (override != null) {
					 ...
					 * */
					addPartialMarkupRendererFilter(override,
							source.getRowType(), obj, name, rowIndex,
							columnIndex, index);
				} else {
					...
					if (!String.class.equals(model.get(name).getClass())
							&& !Number.class.isAssignableFrom(model.get(name)
									.getClass())) {
						Translator<Object> translator = translatorSource
								.findByType(model.get(name).getPropertyType());
						if (translator != null) {
					 ...
					 * */
					addPartialMarkupRendererFilter(val, source.getRowType(),
							obj, name, rowIndex, columnIndex, index);
				}
				columnIndex++;
			}
		}
		...
		/**
		 * Even if it will be done once again in
		 * AjaxComponentEventRequestHandler , we must call
		 * partialRenderer.renderPartialPageMarkup() here to "flush" the
		 * PartialMarkupRendererFilters that we've added into the JSONArray ! It
		 * would be great if we could tell the partialRenderer that the job have
		 * already been done ...
		 * */
	 ...
	 * This is called in the DataTable component, when the datas are loaded by
	 * ajax.
	 * 
	 * @throws IOException
	 */
	public JSONObject sendResponse(Request request, GridDataSource source,
		...
		/**
		 * Filter available data in a normal mode. For ajax mode, we give the
		 * opportunity to the developer to filter data on server-side
		 */
		if (!mode) {
			if (InternalUtils.isNonBlank(request
					.getParameter(DataTableConstants.SEARCH)))
				s = filterData(source);
		}
	 ...
	 * 
	 * @param override
	 *            , the block or the value to render
	 * @param type
	 *            , the type of the object to put inside the environment
	 *            service. Required to render an item inside a loop
	 * @param value
	 *            , the value of the object to put inside the environment
	 *            service. Required to render an item inside a loop
	 * @param rowIndex
	 ...
	 * @param globalIndex
	 *            , the global index iteration
	 * */
	public void addPartialMarkupRendererFilter(final Object override,
			final Class type, final Object value, final String columnName,
			final int rowIndex, final int columnIndex, final int globalIndex) {

		final RenderCommand renderCommand = typeCoercer.coerce(override,
				RenderCommand.class);
					...
					public void renderMarkup(MarkupWriter writer,
						...
						RenderCommand forZone = new RenderCommand() {
							public void render(MarkupWriter writer,
									RenderQueue queue) {
								// Create an element to contain the content for
								// the zone. We give it a mnemonic
								// element name and attribute just to help with
								// debugging (the element itself is discarded).

								final Element zoneContainer = writer
										.element("ajax-partial");

								ajaxFormUpdateController
										.setupBeforePartialZoneRender(writer);
								/**
								 * propagate the current item and the row index
								 * of the loop into the container this is to
								 * allow the block to use it when rendering
								 * itself
								 * */
								rowParam.set(value);
								...
								queue.push(new RenderCommand() {
									public void render(MarkupWriter writer,
											RenderQueue queue) {
										writer.end(); // the zoneContainer
														// element

										// Need to do this Ajax Form-related
										// cleanup here, before we extract the
										// zone content.

										ajaxFormUpdateController
												.cleanupAfterPartialZoneRender();

										String zoneUpdateContent = zoneContainer
												.getChildMarkup();

										if (!(override instanceof Block)) {
											/**
											 * Must check JSONArray's length
											 * because partialRenderer.
											 * renderPartialPageMarkup() is done
											 * twice (see
								...
								// Make sure the zone's actual rendering command
								// is processed first, then the inline
