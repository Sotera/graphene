...
public class AjaxUploadStack implements JavaScriptStack {
	...
	public AjaxUploadStack(
			...
			@Symbol(JQuerySymbolConstants.SUPPRESS_PROTOTYPE) final boolean prototype,
		...
		final String path = String.format(
				"${assets.path}/components/upload/jquery.fileuploader%s.js",
				minified ? ".min" : "");

		javaScriptStack = F
				.flow(path,
						String.format(
								"${assets.path}/components/upload/upload%s.js",
								prototype ? "-jquery" : "")).map(pathToAsset)
				.toList();

		final Mapper<String, StylesheetLink> pathToStylesheetLink = F.combine(
				pathToAsset, JQueryUtils.assetToStylesheetLink);
		cssStack = F.flow("${assets.path}/components/upload/fileuploader.css")
