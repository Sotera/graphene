...
//	height:'auto',
	...
	fromAdvancedSearch:false,
        prevLoadParams: {searchValue: null, value: null, prevValue: null}, // MFM store the calling params for later Pivot
	tbar:null,
//	items: [],
	...
	constructor:function(config)
	{
		...
		config.tbar = Ext.create("DARPA.PBCSToolbar", { institution: this.institution });
		...
		//OLD this.InfoVis = Ext.create("DARPA.InfoVis", {});
                ...
                this.GraphVis = Ext.create("DARPA.GraphVis", { // in js/shared/cytoGraphSubs.js
                	id: config.id + "-PBcygraph" , // id here is the id of the containing DOM element for cytoscape to put it into
                ...
                // MFM2
                this.graph1HopStore = Ext.create('Ext.data.Store', {    // only created once per instance
                    proxy: {
                        type:'ajax',
                        model:'EntityGraph',  
                        timeout:120000,
		...
		var legend = Ext.create("Ext.panel.Panel", {
			...
			items:GLegend.getLegend()
                ...
                var filterSettings = Ext.create("DARPA.FilterSettings", {id:config.id + '-PBGraphFilter'});
                
                // Have to delay this call a bit since the Graph Frame (see PB_Frame) is not defined yet
                var wtimeout2 = window.setTimeout(function() {
                    window.clearTimeout(wtimeout2);
                    filterSettings.setGraph(self);  // MFM test
			...
			{   // MFM
                            width: 'auto', 
                            height: 'auto',
                            id: config.id + '-PBcygraph',  // This is the dom element that contains the graph.  
                            flex:1 
                        });
				...
				items:[
					Ext.create("Ext.panel.Panel", {
						...
						layout:'fit',
						items: [
                                                    Ext.create("DARPA.PBNodeDisplay", {
...
//					legend
    ...
    appendTabTitle: function(appendText) {
        ...
        if (appendText && appendText.length > 0) {
            // DEBUG
            //console.log("eg appendTabTitle: BEFORE origTitle = " + p.originalTitle + ", title = " + p.title);
            
            if (p.originalTitle == undefined || p.originalTitle == null) {
...
//                                if (pb2) {
//                                    pb2.updateText(graph.nodes.length + " nodes");
//                                    pb2.reset();
//                                }
                                ...
                                //    {'Tab':'PB Graph', 'searchResults': { 'nodesFound': graph.nodes.length.toString() }});
                            }
			}
		});
	},
	...
	{            
            ...
            if (self.json != null && self.GraphVis.getGv() != null) {				
                    self.GraphVis.showGraph(self.json, searchValue);	// display the graph
            }        
	},
       	applySettings:function() {
        	...
        	var self = this;
		self.load(self.prevLoadParams.value); // this is the last one we loaded
...
//		var actions = self.getNodeDisplay().getActions();
		...
		// retrieve the name/value pairs in node.data().attrs array using
...
/*			
PWG - You can't select a hidden node to unhide it! This needs to be a menu option.
			Ext.create("Ext.Button",
			{
				text:'UNHIDE', 
				listeners:{
...
/*			
Again, needs to be a menu option. Should not have to select a node in order to unpivot.
			Ext.create("Ext.Button",
			{
				text:'UNPIVOT',
				listeners:{
...
*/			
			Ext.create("Ext.Button",
			{
				...
				listeners:{
					'click':function(b) {
						self.expand(node); // expand out 1 hop from this node
						self.popup.close();
					}
...
//		var pivot = Ext.getCmp(self.id + '-NodeDisplay-Actions-PIVOT');
...
*/        
	...
	// assumes tbe node type is entity for now
		var self=this;
                self.prevLoadParams.type = 'customer'; // FIXME make this a generic term
	...
	unhide:function()
	{
		...
		if (self,prevNode) {
			...
			//    {'Tab':'PB Graph', 'action': 'UNHide', 'selectedNode': self.prevNode.name });
		}; // if prevNode
	...
	{
		var self=this;  
                if(self.prevLoadParams.type=='customer') { // FIXME: make this a generic term
	...
	{	
		...
		graphStore.proxy.extraParams.todt	= toDate;
                if (intype == null || intype.length == 0) {
                    intype = "customer"; // FIXME: make this a generic term
                }
		...
		graphStore.load(
		{
			...
			callback: function(records,operation,success) {
                            ...
                            if (success == false) {
                                ...
                                // Update title to display the communication id value and value of nodes found 
        ...
        doPBNumberExpand: function(intype, pb, node, opts)
        {
            var self = this;
            // DEBUG
            //console.log("doPBNumberExpand: intype = " + intype + ", node.name = " + node.name);
            ...
            if (pb) {
                pb.wait({
                    ...
                    duration: 60000,
                    increment: 10,
                    text: 'Expand ' + node.data().name + ' Searching ...',  // MFM2
                    scope: this,
                    fn: function() {
                        pb.updateText(".");
                    }
            ...
            self.loadOneHop(intype, node, pb, null, null );   // last 2 params are placeholders for start and end date
        ...
        unexpand:function(node)
	{
        ...
        applyFilter: function(searchItems, amount, fromDate, toDate) {            
            ...
            var self = this;
            //applyFilter(self, searchItems, amount, null, null); // MFM from and todates set to null until entity graph has temporal data 
	...
	// passes us the data associated with the node
		...
		if (data.idType == 'customer') { // FIXME: make this a generic term
			showEntityDetails(data.idVal); // global in the html code
