		...
		.define(
				"DARPA.PBGraph",
				{
						...
						var wtimeout2 = window
								.setTimeout(
										function() {
											...
											filterSettings.setGraph(self); // MFM
																			// test
											filterSettings
					...
					pivot : function(node) {
						...
						var self = this;
						self.prevLoadParams.type = 'customer'; // FIXME make
																// this a
																// generic term
					...
					unpivot : function() {
						var self = this;
						if (self.prevLoadParams.type == 'customer') { // FIXME:
																		// make
																		// this
																		// a
																		// generic
					...
					loadOneHop : function(intype, node, pb, fromDate, toDate) {
						...
						if (intype == null || intype.length == 0) {
							intype = "customer"; // FIXME: make this a
													// generic term
						}
								...
								.load({
									...
									callback : function(records, operation,
										...
										if (success == false) {
											...
											// communication id value and value
											// of nodes found
					...
					showDetail : function(data) {
						...
						if (data.idType == 'customer') { // FIXME: make this
															// a generic term
