Ext.define("DARPA.AccountList", {
	...
	columns: [
		{header: 'Account', width:200,  dataIndex: 'account'},
		{header: 'Options', width:120, 
		 renderer:function(value,metadata,record) {
			var accno = ""+record.raw[0];
			//var tablink="showLedger('" + accno + "','" + this.entityId + "','" + this.institution.replace(" ", "_") + "');";
                        tablink="showLedger('" + accno + "','" + this.entityId + "');";
                      
			return ("<button style='font-size:75%;' onclick=" + tablink + ">SHOW LEDGER</button>");
		}}
	],
	store:null,
	constructor:function(config) {
		...
		for (var i = 0; i < accs.length; ++i) {
			dat.push([accs[i]]); // note the extra square brackets to make it an array of 1-element arrays
                }
...
Ext.define("DARPA.EntityPropertiesGrid", {
	...
	columns: [
		{header: ' ', width:100}, // spacer
		{header: 'Type', width:200,  dataIndex: 'name'},
  ...
  function loadFamily(family, property, values, target)
  {
   ...
   Ext.define("DARPA.EntityDetailsPanel", {
	...
	items:[],
	// We have to do this in a constructor or we will get the same items in every instance
	constructor:function(config) {
		this.items = [
			Ext.create("DARPA.AccountList", {entity:config.entity, institution:config.institution}),
			Ext.create("DARPA.EntityPropertiesGrid", {entity:config.entity, institution:config.institution, flex:1})
		];
		this.callParent(arguments);
		return this;
	}	
   ...
   Ext.define("DARPA.EntityDetailsFrame",  {
	...
	constructor:function(config) {
			...
			items:[
				{
					...
					//value:config.entityname
                                        value: "INSTITUTION: " + config.institution + " | Entity: " + config.entityname
				}
			]
			}
		);
		...
		var showgraphbutton = Ext.create("Ext.Container", {
			...
			hidden:true, // going to try preloading the graph so not need this button
			flex:1,
			height:24,
			title:false,
			layout:{
		...
		this.items = [
			topPanel,
			Ext.create("DARPA.EntityDetailsPanel", {flex:1, width:800,margin:'10 0 0 0', entity:config.entity, institution:config.institution})
		]; // items
