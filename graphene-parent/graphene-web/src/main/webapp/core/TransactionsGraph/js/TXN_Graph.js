		...
		.define(
				"DARPA.TransactionGraph",
				{
					...
					constructor : function(config) {
						var self = this;
						// MFM Add a View menu to the top toolbar to allow
						...
						this.GraphVis = Ext.create("DARPA.GraphVis", {
							...
							setBusy : function(busy) {
								...
								tab.setStatus(busy ? "CALCULATING LAYOUT"
										: "CALCULATED LAYOUT");
							}

						});
						this.graphStore = makeTXNGraphStore(); // made this
																// part of the
																// object and
																// only created
																// once per
						...
						var graphContainer = Ext.create("Ext.Container", {
							width : 'auto',
							height : 'auto',
							id : config.id + '-TXNcygraph', // This is the dom
															// element that
															// contains the
															// graph.
							flex : 1
						...
						var details = Ext.create("Ext.tab.Panel", {
							...
							items : [ Ext.create("Ext.panel.Panel", {
								...
								items : [ Ext.create("DARPA.TXNGNodeDisplay", {
					...
					/*
					 * // replaced by getProgressBar for consistancy in the API
					 * between graphs getStatus:function() { var self=this;
					...
					load : function(accountList) {
						...
						graphStore.proxy.url = Config.transferGraphCSUrl
								+ accountList[0];
						...
						// DEBUG
						// console.log("load: graphStore.proxy.url = " +
						// graphStore.proxy.url);
						// console.log("self.prevLoadParams.searchNumber set to
						// " + self.prevLoadParams.searchNumber);

						self.prevLoadParams.prevNumber = self.prevLoadParams.number;
						self.prevLoadParams.number = accountList[0]; // new
																		// number
						...
						/*
						 * var pb = Ext.getCmp("TGgraphpgb"); if (pb) {
						 * pb.wait({ interval: 1000, duration: 90000, increment:
						 * 10, text: 'Searching ...', scope: this, fn:
						 * function() { pb.updateText("."); } }) }
						 */
						graphStore
								.load({
									scope : this, // ?
									callback : function(records, operation,
										...
										// var pb2 =
										// Ext.getCmp(self.id+"-TGgraphpgb");

										if (success == false || records == null
												|| records.length == 0) {
											if (success == false)
												this
														.setStatus("SERVER ERROR REQUESTING GRAPH");
											else if (records == null)
												this
														.setStatus("SERVER RETURNED NULL GRAPH");
											else if (records.length == 0)
												this
														.setStatus("SERVER RETURNED EMPTY GRAPH");

											// if (pb2) {
											// pb2.updateText("Search failed due
											// to server error.");
											// pb2.reset();
											// }
											self.clear();
											...
											if (self.json
												...
												this
														.setStatus("NO DATA FOUND TO PLOT");
												// self.clear(); // don't clear
												// what is already shown
											} else {
												self.clear();
												self
														.showjson(self.prevLoadParams.number);
											}
					...
					showJson : function() {
						...
						if (self.json != null && self.GraphVis.getGv() != null) {
							utils.setBlink(this, true); // the graph code will
														// turn this off when
														// layout is complete
							self.GraphVis.showGraph(self.json, searchNumber); // display
																				// the
																				// graph
						}
					},
					...
					nodeMouseOver : function(node) {
						...
						if (!self.mouseOverPopUp) {
							self.mouseOverPopUp = Ext.create("Ext.Window", {
								...
								// TODO: uncomment when right-click is available
					...
					nodeClick : function(node) {
						...
						nodeDisp.enablePivot(true); // currently all nodes are
													// accounts
					...
					nodeEnter : function(node) {
						// TODO: This does not work because for some reason self
						// was initiated as the window instead of
					...
					showDetail : function(data) {
						...
						// TODO: We are showing the customer here, not the
						// account. Need to create a function for this.

						for (var i = 0; i < data.attrs.length; ++i) {
							if (data.attrs[i].key == 'entityId') {
								showEntityDetails(data.attrs[i].val);
								// TODO: set a flag to load the ledger when the
					...
					pivot : function(node) {
						...
						self.prevLoadParams.prevNumber = node.data().name; // TODO:
																			// check
					...
					graphOneHopStoreInit : function() {
						var graph1HopStore = Ext.getCmp('TGgraph1HopDataStore'); // MFM
																					// - no
																					// need
																					// to
																					// recreate
								...
								.load({
									...
									callback : function(records, operation,
										...
										if (success == false || records == null
											...
											// communication id and number of
											// nodes found
											// self.updateTitle(graph.nodes.length,
											// self.prevLoadParams.number );
										}
										if (mbox) {
											mbox.close();
										}
					...
					doTGNumberExpand : function(intype, pb, node, butid) {
						var self = this;
						// DEBUG
						console.log("doTGNumberExpand: intype = " + intype
						...
						self.loadOneHop(intype, node, pb, null, null, butid); // last
																				// 2
																				// null
																				// params
																				// are
					...
					unexpand : function(node, butid) {
						// DEBUG
						// console.log("UNexpand: node # = " +
						// node.data().name);
						var self = this;

						self.GraphVis.unexpand1Hop(node, butid); // display
																	// the graph
						self.nodesExpanded--;
					...
					applySettings : function() {
						...
						var self = this;
						self.load([ self.prevLoadParams.number ]); // this is
																	// the last
																	// one we
																	// loaded
...
function makeTXNGraphStore() {
	var graphStore = Ext.create('Ext.data.JsonStore', {
		proxy : {
			...
			model : 'GraphTXN',
			timeout : 120000,
			// url:'http:/.../rest/graphml/calls/3495199?degree=3',
			url : '',
			reader : {
				type : 'json',
			// record:'graph',
